...
版本：0.6.0 / W3；日期：2026-09-21；状态：**实际开发进行中；新增离线与真实 CST 证据，完整 Agent/人工验收未通过**。
...
| 能力 | 实现/证据 | 未完成边界 |
|---|---|---|
| 3D选择与检查 | 图中/列表选实体、材料/bbox、隐藏/透明；实际线缆路由中心线 | 不把中心线当实际截面实体或求解网格 |
| 原理图负载 | 元件符号、地、探针、实际引脚表；RES双精度属性编辑并保留Net | 拖拽改线和全部器件库未实现 |
| 设置 | tmax 250→251→250、频段min 0→0.01→0实机通过；原生单位核对 | 波形函数/幅值详细编辑、求解器类型编辑未接入 |
| 手动/Pi接续 | 222实际Pi读手动75Ω→改另端60Ω→再读回→UI跟随→保存重开 | 偶极子同样实际完成r1→1.2并保持L82；两例人工验收待用户 |
| 版本隔离 | 期望工程/SHA、稳定op编号、重复收据、旧值不覆盖 | 不称所有通用宏都具备完整事务回滚 |
| 会话持久 | 已绑定CST的草稿重启保留；标题与目录ID分离 | Pi原始JSONL不移动 |
| 结果刷新 | 模型与结果有效性分开；结果变化隐藏旧曲线 | 无新求解与新完整222 V/I |
