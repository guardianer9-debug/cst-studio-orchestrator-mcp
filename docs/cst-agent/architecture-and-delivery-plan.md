...
|---|---|
| 文档编号 | CST-ARCH-001 |
| 版本 / 轮次 | 0.4.0 / D1（用户调整案例范围与连续交付方式；R1审查保留） |
| 状态 | SCOPE_UPDATED：以指定案例完整复现为终点，尚未实施验收 |
| 日期 | 2026-09-20 |
...
| S1更新基线 | 远端 `5a0a00456acdbe740c43313de344cea25118bdfe`，承接同一worktree的选例补充；当前为本地未提交修改 |
| 配套文件 | [功能矩阵](capability-matrix.md)、[决策与审查记录](review-log.md)、[审查任务](CODEX_REVIEW_TASK.md) |
| 本轮授权 | 方案更新与只读核对；不开发、不启动CST/Provider、不求解；本次修改不提交推送 |

**2026-09-20当前决定：** 项目负责人要求持续实现和排错，直至指定的222孔缝腔体线缆耦合及已选官方算例能够自动化复现。P1A/P1B是内部技术检查点，不是每包交付后暂停等“继续”的边界。砖/RLC可以用于定位问题，但不代替真实案例。本次仍只更新方案和只读资料，不执行开发、CST、Provider或仿真；本次修改未提交/推送。旧R1首包范围和预算由第14.1节新口径替代。
...
以用户指定的当前`222.cst`及其必要依赖为参考，在新工程中自动重建“孔缝腔体的线缆耦合”；原件只读保留。它与此前失败的派生实验工程不是同一基准。可先在专用参考副本读回以冻结规格，再从空工程复建；只打开或复制原工程不能算重建完成。
