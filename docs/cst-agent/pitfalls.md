...
| 问题 | 原因 | 修复 | 复测与适用条件 |
|---|---|---|---|
| 新测试导入旧安装包 | 默认Python路径仍指原checkout | 测试显式PYTHONPATH指向实施src | 保留首次collection失败；后续全量回归 |
| 工作台服务子进程退出 | `python -m`入口内__name__为__main__ | 使用明确模块名启动 | 元数据服务和实际MCP通过；首次日志保留 |
| 健康超时可能产生双所有者 | CST同步调用期间HTTP事件循环忙 | 超时后验证原PID/创建时间/命令，复用活服务 | 禁止因忙自动启动第二CSTClient |
| PiDeck源码文本测试失败 | PowerShell/Python写回引入CRLF | 改动文件保持LF，保留测试断言 | 全量2138通过、2跳过 |
| 新桌面单测返回null | fake execFile缺少Node自定义promisify语义 | 测试替身按真实stdout/stderr契约返回 | 跨会话和路径逃逸测试通过，未放宽断言 |
| 编辑后UI仍选旧版本 | 旧构建未消费后端active_project | 返回并选择实际新工程路径 | 需以实际加载构建验证，旧窗口截图保留 |
| Pi真实读回未到工具阶段 | Provider队列满/连接/内部服务错误；一次组合检索返回空 | 保留3次尝试，第三次给单关键词并沿用legacy协议 | 不升级为Agent通过，不更换Provider或无界重试 |

W2收尾回归：修改前读回会改变CST隐式选中对象。会话模式下通用原理图写入必须指定受支持的target_name；Net等操作使用已有专用工具，避免读回后误改另一个对象。新增回归证明在选择不明确时拒绝执行、不生成模型版本。

## W3：接续使用中的实际修复

| 问题 | 原因 | 修复 | 复测/适用条件 |
|---|---|---|---|
| 有工程但会话入口重启消失 | 上游清理所有未发送草稿 | SessionCatalog保留已绑定CST目录的草稿 | 原会话ID从事前记录恢复，工程未重建；实际窗口恢复和单测通过 |
| 首次草稿修复仍不生效 | Catalog启动早于ProjectStore延迟加载 | 读取持久项目映射进行绑定核验 | 保留两次失败/恢复记录；不提前改全局启动框架 |
| 频段调用失败 | 错用VBABuilder.call的无参数签名 | 使用现有set_double | 原失败版本保留；0→0.01→0 GHz实机通过 |
| Agent改完后界面停在旧版本 | 选择只保留旧路径，没有跟随后台头版本 | 同一会话active_project更新；历史选择可固定，迟到请求失效 | 222、偶极子的实际Agent修改均被界面读取；纯选择规则回归通过 |
| 结果变化可能使整个模型消失 | 同一个有效性字段同时约束CAD和曲线 | 模型SHA与结果目录指纹分开；旧曲线失效而模型可保留 | 结果变化回归通过，不把mtime视为数值归属证明 |
| Provider中断与缺少技能路径 | 网络/队列故障，以及Agent猜测可选技能位置 | 保留操作ID，核对收据/实际值；项目指引明确已配置MCP，无需猜技能路径 | 222两请求、偶极子六请求后完成；未增加Provider或重建工程 |
| Agent总结和原生保存不一致 | 将“未重新导入工程”说成“未创建副本” | 以编辑收据/新版本文件为准，记录说明差异 | 原始回复完整保留，不将自然语言总结作为工程事实来源 |

只读刷新不再覆盖已有版本标签和复现类别：修改变体保持modified_variant，已有操作标签保留；新增回归验证。历史快照不改写，当前会话索引按真实操作收据补齐类别。
